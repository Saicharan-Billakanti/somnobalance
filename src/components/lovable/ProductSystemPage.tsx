...
import { ArrowLeft, ArrowRight, Minus, Pause, Play, Plus, Search, Volume2, type LucideIcon } from "lucide-react";
    ...
    >
      <section className="mx-auto grid max-w-[1500px] gap-8 px-5 pb-10 pt-10 md:px-10 lg:grid-cols-[1.45fr_1fr_0.55fr] lg:px-16">
        <div className="min-w-0">
          <div className="relative aspect-square overflow-hidden rounded-md bg-lovable-muted">
            <Image src={product.image} alt={text.name} width={1200} height={1200} className="h-full w-full object-cover" />
            <div className="absolute bottom-10 left-10 hidden text-[11px] uppercase leading-[2] tracking-[0.25em] text-lovable-primary md:block">
              {config.galleryTagline.map((line, i) => (
                <span key={i}>
                  {line}
                  <br />
                </span>
        ...
        <div className="py-2 lg:py-4">
          <p className="text-[10px] uppercase tracking-[0.25em] text-lovable-muted-foreground">
            {config.systemRoleLabel}
          </p>
          <h1 className="mt-5 font-sans text-4xl font-medium leading-none md:text-5xl">{text.name}</h1>
          ...
          <div className="mt-6 flex items-end gap-3">
            ...
            <span className="pb-1 text-[10px] text-lovable-muted-foreground">{dict.shop.inclVatShipping}</span>
          </div>
          ...
          <div className="mt-7 flex gap-3">
            ...
            >
              <span>{added ? `${quantity} ${dict.shop.addedToCart}` : dict.shop.addToCart}</span>
          ...
          {text.specs && (
            <div className="my-7 grid grid-cols-3 gap-3 border-y border-lovable-border py-5 text-[10px]">
              <span className="flex items-center gap-2">{text.specs.format}</span>
              <span className="flex items-center gap-2">{text.specs.type}</span>
              <span className="flex items-center gap-2">{text.specs.usage}</span>
            </div>
          )}

          <p className="text-xs leading-6 text-lovable-muted-foreground">{text.description}</p>
        </div>
        ...
        <div className="mx-auto grid max-w-[1500px] items-center gap-8 px-5 md:px-10 lg:grid-cols-[280px_1fr_180px] lg:px-16">
          <div>
            ...
            <h2 className="mt-8 font-lovable-serif text-3xl leading-tight md:text-4xl">
              {config.howToUseTitle[0]}
              <br />
              {config.howToUseTitle[1]}
            </h2>
          </div>
          <div>
          ...
          <p className="border-l border-lovable-border pl-8 font-lovable-serif text-xl leading-tight">
            {config.howToUseNote[0]}
            <br />
            {config.howToUseNote[1]}
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-[1500px] lg:grid-cols-[0.9fr_1.05fr_0.75fr]">
        <div className="relative min-h-[520px]">
          <Image src={product.image} alt={text.name} fill className="object-cover" />
        </div>
        <div className="px-7 py-12 md:px-12">
          ...
          <ul className="mt-6 space-y-7 text-xs leading-5">
            {text.details.map((d, i) => {
              const Icon = config.detailIcons[Math.min(i, config.detailIcons.length - 1)];
              return (
        ...
        <div className="border-l border-lovable-border px-7 py-12">
          {text.ingredients && (
            <div className="rounded-md border border-lovable-border bg-lovable-card p-6">
              <p className="text-[9px] uppercase tracking-[0.18em]">{dict.shop.ingredients}</p>
              <p className="mt-5 text-[10px] leading-5 text-lovable-muted-foreground">{text.ingredients}</p>
            </div>
          )}
      ...
      <section className="border-y border-lovable-border bg-lovable-secondary/35 px-5 py-14 md:px-10 lg:px-16">
        <div className="mx-auto max-w-[1500px]">
          <p className="text-[10px] uppercase tracking-[0.25em] text-lovable-muted-foreground">System</p>
          <p className="mt-3 max-w-lg font-lovable-serif text-2xl italic leading-snug text-lovable-primary">
            {config.nextCompanionLabel}
          </p>
        </div>
      </section>

      <section className="relative flex min-h-[285px] items-center overflow-hidden">
        <Image src="/brand/pdp-bedroom.jpg" alt="A calm bedroom lit by a warm bedside lamp" fill className="object-cover" />
        <div className="relative z-10 px-8 text-lovable-primary-foreground md:px-16">
          ...
          <p className="mt-2 text-[11px] uppercase leading-6 tracking-[0.25em]">
            {config.brandCloseTagline[0]}
            <br />
            {config.brandCloseTagline[1]}
          </p>
        ...
        <p className="absolute right-[12%] top-[25%] z-10 rotate-[-10deg] font-lovable-serif text-2xl italic text-lovable-primary-foreground/80">
          {config.brandCloseNote[0]}
          <br />
          {config.brandCloseNote[1]}
          <br />
          {config.brandCloseNote[2]}
        </p>
      </section>
    </div>
  );
