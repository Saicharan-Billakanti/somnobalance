...
export default function CommissionsPage() {
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl text-ink">Commissions</h1>
      <p className="mt-2 text-ink/70">
        Commission becomes available after the configured approval period. You cannot edit these records.
      </p>

      {commissions.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-mauve/10 bg-white/60 p-10 text-center">
          <p className="text-ink/70">No commissions yet.</p>
          <p className="mt-1 text-sm text-ink/50">Share your affiliate link to start earning.</p>
        </div>
      ) : (
              ...
              {commissions.map((c) => (
                <tr key={c.orderId} className="border-b border-mauve/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-ink/70">
                    {new Date(c.orderDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                  </td>
