...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return { title: lang === "de" ? "Für mich — SomnoBalance" : "For me — SomnoBalance" };
}
      ...
      <div className="py-14" style={{ background: "var(--color-scroll-2)" }}>
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <div className="flex flex-wrap gap-4">
            ...
            <Link href={`/${lang}/shop`} className="rounded-full border border-mauve/30 px-6 py-3 text-sm text-mauve-dark hover:bg-white/60">
              {dict.forMe.ctaShop}
            </Link>
            <Link href={`/${lang}/about`} className="rounded-full border border-mauve/30 px-6 py-3 text-sm text-mauve-dark hover:bg-white/60">
              {dict.forMe.ctaAbout}
            </Link>
          </div>
        </div>
