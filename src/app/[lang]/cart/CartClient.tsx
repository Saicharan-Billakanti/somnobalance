...
export function CartClient({ lang, dict }: { lang: Locale; dict: Dictionary }) {
  ...
  // empty-cart early return used to sit before these useState calls, which
  // would throw/mismatch hook order the moment the cart went from empty to
  // non-empty. Moved above the early return to fix that.
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6">
        ...
        {lines.map((line) => {
          ...
          return (
            <div
              ...
              className="space-y-3 rounded-2xl border border-mauve/10 bg-white/50 p-4"
            >
              <div className="flex items-center gap-5">
                <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-white">
                  <Image src={product.image} alt={text.name} fill sizes="96px" className="object-contain" />
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="w-16 rounded-lg border border-mauve/20 bg-white px-2 py-1 text-center font-bold text-ink"
                  />
                </div>
              ...
              {isHighQty && (
                <div className="animate-fade-in flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-amber-300 bg-amber-50/80 p-3.5 text-xs text-amber-950">
                  ...
                  <Link
                    ...
                    className="shadow-xs whitespace-nowrap rounded-full bg-amber-700 px-4 py-1.5 text-xs font-bold text-white transition hover:bg-amber-800"
                  >
        ...
        <div className="flex-1">
          ...
          <div className="mt-1 flex max-w-sm gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-mauve/20 bg-white px-3 py-1.5 text-sm uppercase text-ink placeholder:normal-case placeholder:text-ink/40 focus:outline-none focus:ring-1 focus:ring-mauve"
            />
            {appliedCoupon ? (
              <button
                type="button"
      ...
      <div className="mt-10 flex flex-col gap-4 rounded-2xl border border-mauve/10 bg-white/50 p-6 sm:flex-row sm:items-center sm:justify-between">
